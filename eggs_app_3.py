...
# 1. メニューデータの完全定義（さいたま新都心店グランドメニュー＆最新キッズメニュー）
menu_data = {
    "1. パンケーキ": {
...
# リセットボタン（クリックされたらコールバックで安全に0に戻す）
st.button("🔄 選択をすべてリセットする", use_container_width=True, type="secondary", on_click=handle_reset)
...
for category, items in menu_data.items():
    with st.expander(category):
        for item_name, price in items.items():
            ...
            with col2:
                # keyにセッション変数を直接紐付けることで、自動保存・自動復元・リセットが完全に1つのデータで同期します
